...
class TrainingMode:
    ...
    def __init__(self, screen=None):
        ...
        # Use provided screen if available, otherwise create a new one
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    if self.show_camera_selection:
                        ...
                        # Restart game
                        self.__init__(self.screen)  # Preserve the screen parameter
                    elif not self.game_active:
                        # Start game
                        self.start_game()
                
                # Handle mirror mode toggle with the 'M' key
                elif event.key == pygame.K_m:
                    # Toggle mirror image mode
                    self.mirror_image = not self.mirror_image
                    # Show notification
                    self.show_notification(f"Mirror Mode: {'ON' if self.mirror_image else 'OFF'}")
                    print(f"Mirror mode toggled: {'ON' if self.mirror_image else 'OFF'}")
                        
                # Toggle camera display with 'C' key
                elif event.key == pygame.K_c:
                    self.show_camera = not self.show_camera
                    self.show_notification(f"Camera Display: {'ON' if self.show_camera else 'OFF'}")
                ...
                # Camera switching with left/right arrow keys
                    ...
                    if self.show_camera_selection or pygame.key.get_mods() & pygame.KMOD_SHIFT:
                        ...
                        if self.select_camera(new_index):
                            camera_name = self.available_cameras[new_index]["name"]
                            self.show_notification(f"Camera switched to: {camera_name}")
    ...
    def draw_notification(self):
        """Draw temporary notification message if active."""
        ...
        alpha = 255 - int(255 * (elapsed / self.notification_duration))
        ...
        bg_surface.fill((0, 0, 0, int(alpha * 0.7)))  # Semi-transparent black
    ...
    def draw_hud(self):
        ...
        if self.game_active:
            ...
            # Add mirror mode status indicator with high visibility
            mirror_status_bg = pygame.Rect(SCREEN_WIDTH//2 - 200, SCREEN_HEIGHT - 70, 400, 30)
            pygame.draw.rect(self.screen, (50, 50, 50), mirror_status_bg)
            pygame.draw.rect(self.screen, (100, 100, 100), mirror_status_bg, 2)
            
            mirror_color = GREEN if self.mirror_image else RED
            mirror_status = self.font_small.render(f"Mirror Mode: {'ON' if self.mirror_image else 'OFF'} (Press M to toggle)", True, mirror_color)
            self.screen.blit(mirror_status, (SCREEN_WIDTH // 2 - mirror_status.get_width() // 2, SCREEN_HEIGHT - 65))
    ...
    def draw_camera_feed(self):
        ...
        if self.camera_frame is not None and self.data_panel is not None:
            ...
            # Add mirror mode indicator directly on the camera feed
            mirror_label = self.font_small.render(f"Mirror: {'ON' if self.mirror_image else 'OFF'}", True, GREEN if self.mirror_image else RED)
            self.screen.blit(mirror_label, (SCREEN_WIDTH - mirror_label.get_width() - 10, 40))
